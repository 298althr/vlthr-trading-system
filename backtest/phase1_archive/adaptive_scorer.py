...
def score_signal_adaptive(symbol: str, enriched_df: pd.DataFrame,
    ...
    # Load per-symbol RSI threshold
    from portfolio_config import get_symbol_params
    sym_params = get_symbol_params(symbol)
    rsi_thresh = sym_params.get("rsi_threshold", 40.0)

    # Determine direction based on per-symbol strategy
    strategy = sym_params.get("strategy", "trend_following")
    ...
    if mult_cap_hit and tech_raw < 60:
