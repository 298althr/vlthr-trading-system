...
==============================
Single entry point for one 15-minute pipeline iteration.
...
from portfolio_config import (SYMBOLS, ACTIVE_SESSIONS, DQS_THRESHOLDS, PORTFOLIO, TELEGRAM_BOT_NAME,
    ...
    DQS_VETO_THRESHOLD, DISABLED_SYMBOLS, get_kelly_v7, MR_BOOST, SYMBOL_ALLOC_CAP,
...
def _get_account_balance(conn):
    """Fetch paper_account true equity (balance + margin_used).
    Phase 1 fix: use (equity + margin_used) instead of balance alone
    so risk capacity reflects true portfolio state."""
    cur = conn.cursor()
    cur.execute("SELECT balance, equity, margin_used FROM paper_account LIMIT 1")
    row = cur.fetchone()
    if not row:
        return 10000.0
    balance = float(row[0] or 10000.0)
    equity = float(row[1] or balance)
    margin_used = float(row[2] or 0)
...
def _fallback_monitor_trades(conn, now):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT id, signal_id, symbol, side, entry_price_actual, sl_price, tp_price,
    ...
    closed = 0
    for row in rows:
        (tid, sig_id, symbol, side, entry, sl, tp, qty, lev, margin_req,
         entry_fee, funding_total, created_at) = row
        ...
        if enriched is None or len(enriched) == 0:
            continue
        live_price = float(enriched.iloc[-1]["close"])
        if live_price <= 0:
            continue
        ...
        # Determine exit reason (same logic as dashboard, minus dynamic SL)
        reason = None
        is_long = side == "LONG"
        ...
        gross_usd = (live_price - entry) * qty if is_long else (entry - live_price) * qty
        gross_pct = ((live_price - entry) / entry) * 100 if is_long else ((entry - live_price) / entry) * 100
        notional = entry * qty
        exit_notional = live_price * qty
        ...
        try:
            cur.execute("""
                ...
                WHERE id=%s
            """, (live_price, reason, gross_pct, gross_usd, close_fee, total_fees,
            ...
            print(f"  [FALLBACK_MONITOR] Closed trade {tid} {symbol} {side}: {reason} "
                  f"@ {live_price:.4f}, net_pnl=${net_usd:.2f}, held {hours_held:.1f}h")
    ...
    for row in pending_rows:
        ...
        try:
            ...
            if sl_mode == "pct":
                ...
                sl_price, tp_price, rr = get_sl_tp(close_price, atr, int(dqs), side=side, regime=regime_val)
            if sl_price is None:
            ...
            cur.execute("""
                UPDATE paper_trades SET
                    ...
                    updated_at = NOW()
                WHERE id = %s
            """, (signal_bar_utc, close_price, close_price, sl_price, tp_price,
        ...
        try:
            ...
            if strategy != original_strategy:
                ...
                if strategy == "trend_following":
                    ...
                    sym_p = get_symbol_params(symbol)
