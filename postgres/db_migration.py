...
def migrate():
    ...
    # ── Phase 2: Bybit dual-execution columns on paper_trades ──
    bybit_cols = [
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_order_id VARCHAR(64)", "paper_trades.bybit_order_id"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_qty NUMERIC(18,4)", "paper_trades.bybit_qty"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_entry_price NUMERIC(18,8)", "paper_trades.bybit_entry_price"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_sl_price NUMERIC(18,8)", "paper_trades.bybit_sl_price"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_tp_price NUMERIC(18,8)", "paper_trades.bybit_tp_price"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_status VARCHAR(20) DEFAULT 'PENDING'", "paper_trades.bybit_status"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_close_price NUMERIC(18,8)", "paper_trades.bybit_close_price"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_pnl_usd NUMERIC(12,4)", "paper_trades.bybit_pnl_usd"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_pnl_pct NUMERIC(8,4)", "paper_trades.bybit_pnl_pct"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_account_balance NUMERIC(12,2)", "paper_trades.bybit_account_balance"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_risk_pct NUMERIC(5,2)", "paper_trades.bybit_risk_pct"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_dollar_risk NUMERIC(12,2)", "paper_trades.bybit_dollar_risk"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_close_time TIMESTAMPTZ", "paper_trades.bybit_close_time"),
        ("ALTER TABLE paper_trades ADD COLUMN IF NOT EXISTS bybit_close_reason VARCHAR(32)", "paper_trades.bybit_close_reason"),
    ]
    for sql, name in bybit_cols:
        try:
            cur.execute(sql)
            conn.commit()
            print(f"[Migration] {name} column verified")
