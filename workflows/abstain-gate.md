...
7. **Integrate into decision pipeline**
   ...
   - **Jul 2026 hardening:** ABSTAIN gate now also checked at reprice promotion (PENDING→OPEN) alongside all other portfolio gates (CorrelationGuard, TradeCountGuard, RiskBudgetLedger, DirectionalGuard)
...
## V2 Calibration Results (Deprecated — Pre-V7)

These results were the basis for the V2 ABSTAIN gate. V7 supersedes this with isotonic calibration.
...
**31 of 42 cells ABSTAIN** (6 symbols x 7 non-overlapping regimes; crisis and liquidity_shock are mutually exclusive with other regimes). Only SOLUSDT + sideways/crisis pass.
...
- `engine/portfolio_orchestrator.py` — `_should_abstain()` + check + reprice gate enforcement
- `engine/v3/orchestrator.py` — inline check
- `engine/test_risk_logic.py` — unittest tests for gate logic
