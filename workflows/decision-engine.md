...
### Portfolio Gates at Reprice Promotion
- All portfolio gates (CorrelationGuard, TradeCountGuard, RiskBudgetLedger, DirectionalGuard) now run at PENDING→OPEN promotion in `_reprice_pending_signals`
- Previously gates only ran at signal generation (Step 4) — reprice path could bypass risk checks
- Trades failing gates at reprice are expired with audit log entries

### Flash-Wick Protection
- **server.cjs**: 10% spike filter — if `|livePrice - entry_price| / entry_price > 0.10`, skip close check that cycle
- **fallback monitor** (portfolio_orchestrator.py): 10% bar-to-bar close price filter — if `|live_price - prev_close| / prev_close > 0.10`, skip trade
- Prevents false SL/TP triggers from anomalous parquet bars or data glitches

### TP/SL Tuning from Closed Trades
- `tune_tp_from_history(conn)` called at pipeline startup
- If ≥20 closed trades with 0 TP hits and >5 TIME_EXITs: tightens TP multipliers by 20%
- Floored at `sl_mult * MIN_RR_FLOOR` to maintain minimum R:R
- Adjusts `RISK_TIERS` in-place (runtime, not config file)

### Immutable Audit Trail
- SHA-256 checksum (16-char) added to `error_log` and `signal_audit_log` inserts
- Enables tamper detection on critical audit records
- DB migration: `ALTER TABLE ... ADD COLUMN IF NOT EXISTS checksum VARCHAR(16)`
...
- `engine/portfolio_orchestrator.py` — reprice gates, flash-wick fallback, TP tuning call
- `engine/portfolio_config.py` — `tune_tp_from_history()`, RISK_TIERS
- `engine/safety_layer.py` — checksum on error_log
- `backend/server.cjs` — flash-wick guard in auto-close
