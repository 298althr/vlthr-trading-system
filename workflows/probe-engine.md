...
## Post-Leakage-Fix Results (Pre-V7)
...
## V7 Context
V7 uses the V3 adaptive scorer with ctx-only weighting (tech 0.00, struct 0.00, ctx 1.00). The probe results above informed this decision — tech and struct domains had negative IC, so they were zeroed out. The isotonic calibration filter (`calibrated_win_prob > 0.48`) builds on top of the ctx-only scorer to achieve 71.9% direction accuracy on filtered trades.
