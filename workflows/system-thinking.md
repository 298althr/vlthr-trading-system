...
## VLTHR V7 Application
- **System boundary:** 6 Bybit perpetuals (5 active — DOGEUSDT disabled), 15m execution, 4h context, Supabase DB
- **Reinforcing loop:** V7 calibration filter -> 54% WR -> confidence -> shadow deploy -> live feedback -> recalibrate -> better filter
- **Balancing loop:** Isotonic calibration filter -> fewer trades (87% filtered) -> less data -> slower calibration -> filter loosens -> more trades
- **Leverage point:** Robustness pillar (11/15) — improving WF decay at 75% split (45.7%) would strengthen the institutional score
- **Emergent property:** 71.9% direction accuracy on filtered trades emerges from isotonic calibration + V3 ctx-only scorer + DQS 85+ veto + crowd LS veto interacting together
- **Unintended consequence:** Calibration filter rejects 87% of base decisions — pipeline trades much less than raw signal generation, but with higher quality
